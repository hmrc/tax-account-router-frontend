...
class TwoStepVerificationServiceSpec extends UnitSpec with MockitoSugar with WithFakeApplication with SpecHelpers {
  ...
  "getDestinationVia2SV" should {
    ...
    "rewrite the location to have the 2SV url and the origin when the continue url is BTA, the user is not registered for 2SV, no strong credentials, has one enrolment, has SA enrolment and throttle chooses optional registration" in new Setup {
      ...
      result shouldBe Some(Locations.twoStepVerification(Map("continue" -> Locations.BusinessTaxAccount.url, "failure" -> Locations.BusinessTaxAccount.url, "origin" -> "business-tax-account", "b2svRegistration" -> "optional", "b2svRule" -> "sa-only")))
    ...
    "rewrite the location to have the 2SV url and the origin when the continue url is BTA, the user is not registered for 2SV, no strong credentials, has one enrolment, has SA enrolment and throttle chooses mandatory registration" in new Setup {
      ...
      result shouldBe Some(Locations.twoStepVerification(Map("continue" -> Locations.BusinessTaxAccount.url, "failure" -> Locations.TaxAccountRouterHome.url, "origin" -> "business-tax-account", "b2svRegistration" -> "mandatory", "b2svRule" -> "sa-only")))
      verify(ruleContext).currentCoAFEAuthority
      verify(ruleContext, times(2)).activeEnrolments
      verify(ruleContext).enrolments
      verify(twoStepVerificationThrottleMock).registrationMandatory(userid)
